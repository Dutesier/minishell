...
int expand_quote(t_shell *shell, t_ast *father, int i)
{
	...
	if (found_dollar && value[j+1] != '\"' && value[j+1] != ' ')
		return (exp_needed(shell, father, ft_dupnoq(father->branches[i]->my_tok->value), i));
...
// Lets just change the whole double quotes string for a str with the variable expanded
